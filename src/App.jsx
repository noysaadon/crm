...
export default function App(){
  ...
  return <>
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className="topbar">
          ...
          {page==='leads'&&<div style={{display:'flex',gap:8}}><button className="btn primary" onClick={()=>setModal({type:'lead',data:null})}>＋ לקוח חדש</button><button className="btn" style={{background:'#f0efeb'}} onClick={()=>setShowImport(true)}>📊 ייבוא אקסל</button></div>}
        </div>
    ...
    {showImport&&<ExcelImport onImport={async form=>{const r=await supabase.from('leads').insert([form]);loadLeads();return r}} onClose={()=>{setShowImport(false);loadLeads()}}/> }
